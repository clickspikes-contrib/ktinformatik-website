...
function thmv_get_theme_version() {
    $thmv_theme = wp_get_theme( 'stratusx' );
    if ( $thmv_theme->exists() )
    return $thmv_theme->get('Version');
}
            ...
            if ($th_screen->id === 'edit-elementor-thhf') {
                ...
                if ( get_user_meta( $user_id, 'thmv_set_hfe_help_dismissed', true ) !== '1' ) { ?>
                    <div class="notice notice-info is-dismissible thmv_hfe_edit">
                        <p><strong><?php _e('Get started quickly with our <a href="https://themovation.helpscoutdocs.com/article/311-custom-header-footer" target="_blank">templates and help guide!</a>', 'stratus'); ?> <span><a href="https://themovation.helpscoutdocs.com/article/311-custom-header-footer" target="_blank">Learn more</a> | <a class="th-dismiss" href="edit.php?post_type=elementor-thhf&thmv-hfe-help-dismissed">Dismiss this notice</a></span></strong></p>
                    </div>
                    <?php
...
function th_masterslider_update_check() {
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        ...
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/revslider.json',
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/masterslider.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/th-widget-pack.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        ...
        if ( is_dir( $th_plugin_dir ) ) {
            // plugin directory found!
            $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
                'https://update.themovation.com/kirki.json',
                $th_plugin_dir.'/'.$th_plugin_slug.'.php',
                $th_plugin_slug
            );
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/envato-market.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }
...
function th_elementor_update_check() {
    ...
    if( ! function_exists( 'elementor_pro_load_plugin' ) ) {
        ...
        if ( is_dir( $th_plugin_dir ) ) {
            // plugin directory found!
            $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
                'https://update.themovation.com/elementor.json',
                $th_plugin_dir.'/'.$th_plugin_slug.'.php',
                $th_plugin_slug
            );
...
function th_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
            ...
            'source'    => 'https://out.themovation.com/th-widget-pack', //get_template_directory() . '/plugins/th-widget-pack.zip', // The plugin source.
            'required'  => true,
        ),
        array(
        ...
        array(
            'name' => 'Envato Market',
            'slug' => 'envato-market',
            'source' => 'https://out.themovation.com/envato-market',
            'required' => true,
        ),
    ...
    // Booked
    if ( get_theme_mod('themo_tgmpa_revslider',false) ){
    ...
    // WooCommerce
    if ( get_theme_mod('themo_tgmpa_woocommerce',false) ){
        array_push($plugins, array(
    ...
    // Groovy Menu
    if ( get_theme_mod('themo_tgmpa_groovy_menu',false) ){
        array_push($plugins, array(
    ...
    // Master Slider
    if ( get_theme_mod('themo_tgmpa_masterslider',false) ){
        array_push($plugins, array(
    ...
    // Formidable Forms
    if ( get_theme_mod('themo_tgmpa_formidable',true) ){
        array_push($plugins, array(
    ...
    if ( get_theme_mod('themo_tgmpa_simple_page_ordering',true) ){
        array_push($plugins, array(
    ...
    // Widget Logic
    if ( get_theme_mod('themo_tgmpa_widget_logic',true) ){
        array_push($plugins, array(
            'name'      => 'Widget Logic',
            'slug'      => 'widget-logic',
            'required'  => false,
